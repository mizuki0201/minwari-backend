...
フロント側、サーバー側、双方において、データを送信する際のバリデーションや、それに付随したエラーハンドリングについての実装が難しいと感じました。  
フォームの入力をどこまでフロント側で制御して、サーバー側ではどのようなバリデーションをかけ、その結果として何を返すべきなのか、といったところが悩ましいポイント、かつ知識不足が故に実装がうまくできなかったと感じるポイントです。  
今後の学習課題として、ユーザーが入力したデータをどのように扱うかを学んでいく必要があると感じております。
